...
function initForms() {
    ...
    if (betaForm && betaStatus) {
        betaForm.addEventListener('submit', (e) => {
            ...
            const env   = (document.getElementById('beta-env')?.value   || '').trim();
            const desc  = (document.getElementById('beta-desc')?.value  || '').trim();
            const logs  = (document.getElementById('beta-logs')?.value  || '').trim();

            const to   = 'cafepulse.network@gmail.com';
            const sub  = encodeURIComponent(`[CafePulse Beta Bug] From ${email}`);
            const body = encodeURIComponent(
...
/* --------------------------------------------------------------------------
   5. Dynamic Platform Switcher (Windows vs Linux)
   -------------------------------------------------------------------------- */
function switchPlatform(platform) {
    ...
    // Add active to clicked button (handle inline click binding)
    if (window.event && window.event.currentTarget) {
        window.event.currentTarget.classList.add('active');
    }

    // Toggle active class on platform content sections
    ...
    // Update Quick Command text box in the sidebar
